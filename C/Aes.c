#include <stdio.h>
#include <stdint.h> 
#include "Aes.h"

static const uint8_t sbox[256] = {
  
};

static const uint8_t Rcon[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};
static const uint8_t inverse_sbox[256] = {
   };

void print_state(uint8_t State[4][4]) {
    for(int ligne = 0; ligne < 4; ligne++) {
        for(int col = 0; col < 4; col++) {
            printf("%02x ", State[ligne][col]); 
        }
        printf("\n");
    }
    printf("\n");
}

void sub_bytes(uint8_t State[4][4]) {
    for (int ligne = 0; ligne < 4; ligne++) {
        for (int col = 0; col < 4; col++) {
            uint8_t byte = State[ligne][col];
            State[ligne][col] = sbox[byte]; 
        }
    }
}

void shift_rows(uint8_t State[4][4]) {
    uint8_t temp;

    temp = State[1][0];
    State[1][0] = State[1][1];
    State[1][1] = State[1][2];
    State[1][2] = State[1][3];
    State[1][3] = temp;

    temp = State[2][0];
    State[2][0] = State[2][2];
    State[2][2] = temp;
    
    temp = State[2][1];
    State[2][1] = State[2][3];
    State[2][3] = temp;

    temp = State[3][3];
    State[3][3] = State[3][2];
    State[3][2] = State[3][1];
    State[3][1] = State[3][0];
    State[3][0] = temp;
}

// faire fois 2 = décaler les bits de un vers la gauche 
// si le bit à gauche est à 1 on fait un xor avec 0x1B (x^8 +x^4 + x^3 + x + 1) pour le réduire sinon rien 
uint8_t mul_02(uint8_t x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1B : 0x00);
}

uint8_t mul_03(uint8_t x) {
    return mul_02(x) ^ x;
}

void mix_columns(uint8_t State[4][4]) {
    uint8_t State_0, State_1, State_2, State_3;

    for(int i = 0; i < 4; i++) {
        State_0 = mul_02(State[0][i])   ^ mul_03(State[1][i])         ^        State[2][i]        ^        State[3][i];
        State_1 =        State[0][i]    ^ mul_02(State[1][i])         ^ mul_03(State[2][i])       ^        State[3][i];
        State_2 =        State[0][i]    ^        State[1][i]          ^ mul_02(State[2][i])       ^ mul_03(State[3][i]);
        State_3 = mul_03(State[0][i])   ^        State[1][i]          ^        State[2][i]        ^ mul_02(State[3][i]);
        
        State[0][i] = State_0;
        State[1][i] = State_1;
        State[2][i] = State_2;
        State[3][i] = State_3;
    }    
}

// RotWord
void RotWord(uint8_t word[4]) {
    uint8_t tmp = word[0];
    word[0] = word[1];
    word[1] = word[2];
    word[2] = word[3];
    word[3] = tmp;
}

void SubWord(uint8_t word[4]) {
    for (int i=0; i<4; i++) {
        uint8_t byte = word[i];
        word[i] = sbox[byte];
    }
}



void keyExp(uint8_t key[16], uint8_t w_output[44][4]) {
    for (int col = 0; col < 4; col++) {
        for (int row = 0; row < 4; row++) {
            w_output[col][row] = key[4*col + row]; 
        }
    }

    uint8_t temp[4];
    for (int i = 4; i < 40; i++) {
        for (int j = 0; j < 4; j++) {
            temp[j] = w_output[i-1][j];
        }

        if (i % 4 == 0) {
            RotWord(temp);
            SubWord(temp);
            temp[0] ^= Rcon[(i/4) - 1];
        }

        for (int j = 0; j < 4; j++) {
            w_output[i][j] = w_output[i-4][j] ^ temp[j];
        }
    }
}

void AddRoundKey(uint8_t State[4][4], uint8_t w_output[44][4], int round) {
    int base = round * 4; 
    for (int col = 0; col < 4; col++) {
        for (int row = 0; row < 4; row++) {
            State[row][col] ^= w_output[base + col][row];
        }
    }
}

void inv_shift_rows(uint8_t State[4][4]) {

    uint8_t temp,temp2;
    
    temp = State[1][3];
    State[1][3] = State[1][2];
    State[1][2] = State[1][1];
    State[1][1] = State[1][0];
    State[1][0] = temp;

    temp  = State[2][0];
    temp2 = State[2][1];
    State[2][0] = State[2][2];
    State[2][1] = State[2][3];
    State[2][2] = temp;
    State[2][3] = temp2;

    temp = State[3][0];
    State[3][0] = State[3][1];
    State[3][1] = State[3][2];
    State[3][2] = State[3][3];
    State[3][3] = temp;
}

void inv_sub_bytes(uint8_t State[4][4]) {
    for (int ligne = 0; ligne < 4; ligne++) {
        for (int col = 0; col < 4; col++) {
            uint8_t byte = State[ligne][col];
            State[ligne][col] = inverse_sbox[byte]; 
        }
    }
}

uint8_t mul_09(uint8_t x) {
    uint8_t x2 = mul_02(x);       
    uint8_t x4 = mul_02(x2);      
    uint8_t x8 = mul_02(x4);      
    return x8 ^ x;                
}

uint8_t mul_0b(uint8_t x) { 
    uint8_t x2 = mul_02(x); 
    uint8_t x4 = mul_02(x2); 
    uint8_t x8 = mul_02(x4); 
    return x8 ^ x2 ^ x;           
}

uint8_t mul_0d(uint8_t x) { 
    uint8_t x2 = mul_02(x); 
    uint8_t x4 = mul_02(x2); 
    uint8_t x8 = mul_02(x4); 
    return x8 ^ x4 ^ x;           
}

uint8_t mul_0e(uint8_t x) { 
    uint8_t x2 = mul_02(x); 
    uint8_t x4 = mul_02(x2); 
    uint8_t x8 = mul_02(x4); 
    return x8 ^ x4 ^ x2;          
}

void inv_mix_columns(uint8_t State[4][4]) {
    uint8_t State_0, State_1, State_2, State_3;

    for(int i = 0; i < 4; i++) {
        State_0 = mul_0e(State[0][i]) ^ mul_0b(State[1][i]) ^ mul_0d(State[2][i]) ^ mul_09(State[3][i]);
        State_1 = mul_09(State[0][i]) ^ mul_0e(State[1][i]) ^ mul_0b(State[2][i]) ^ mul_0d(State[3][i]);
        State_2 = mul_0d(State[0][i]) ^ mul_09(State[1][i]) ^ mul_0e(State[2][i]) ^ mul_0b(State[3][i]);
        State_3 = mul_0b(State[0][i]) ^ mul_0d(State[1][i]) ^ mul_09(State[2][i]) ^ mul_0e(State[3][i]);
        
        State[0][i] = State_0;
        State[1][i] = State_1;
        State[2][i] = State_2;
        State[3][i] = State_3;
    }    
}


